...
6. **Versionamento e publicação são automáticos via semantic-release** — agentes nunca bumpam `package.json`, nunca escrevem no `CHANGELOG.md`, nunca alteram o badge do `README.md` e nunca criam ou enviam tags manualmente. O fluxo (`.github/workflows/release.yml` chamando o workflow reutilizável `.github/workflows/semantic-release.yml`, ambos disparados em todo push na `main`) é: `quality` (typecheck, lint, test) → `release` (semantic-release analisa os commits desde a última tag usando Conventional Commits — `feat:`/`fix:`/`perf:`/`BREAKING CHANGE:` disparam release, `chore:`/`docs:`/`refactor:`/`test:`/`ci:`/`build:` não — e só publica quando há algo relevante) → `docker` (só roda se uma versão nova foi publicada, usa `needs.release.outputs.new-release-version` pra taguear `X.Y.Z`, `X.Y`, `X` e `latest` no GitHub Container Registry, `ghcr.io/welington98/openmonetis`). Quando publica, o semantic-release faz bump do `package.json`, gera as seções do `CHANGELOG.md` (`### Adicionado/Corrigido/Desempenho/Alterado`, a partir dos `feat`/`fix`/`perf`/`refactor` desde a última versão — sem parágrafo introdutório em prosa, é gerado a partir das mensagens de commit) e o badge do `README.md`, commita de volta na `main`, cria a tag `vX.Y.Z` e a GitHub Release — tudo com o `GITHUB_TOKEN` padrão do Actions. O que importa pros agentes é só a mensagem de commit: seguir Conventional Commits (`feat:`, `fix:`, `perf:`, `refactor:`, `chore:`, `docs:`, `test:`, `ci:`, `build:`, com `BREAKING CHANGE:` no corpo quando aplicável) pra que o tipo certo de commit dispare (ou não) a versão certa. Config em `.releaserc.json`.
